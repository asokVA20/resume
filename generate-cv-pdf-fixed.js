...
class FixedCVGenerator {
    ...
    setupDocument() {
        ...
        // Set document properties without problematic metadata
        this.doc.info = {
...
// Main function to generate fixed PDF
function generateFixedCVPDF() {
    try {
        // Load personal data
        const data = JSON.parse(fs.readFileSync('personal-data.json', 'utf8'));
        
        // Create CV generator
        const cvGenerator = new FixedCVGenerator(data);
        
        // Generate PDF
        const doc = cvGenerator.generate();
        
        // Save to file
        const outputPath = 'cv-anthony-okala-fixed.pdf';
        doc.pipe(fs.createWriteStream(outputPath));
        doc.end();
        
        console.log('✅ Fixed CV PDF generated successfully!');
        ...
        console.error('❌ Error generating fixed CV PDF:', error.message);
        throw error;
    }
}

// Export for use in other files
module.exports = { generateFixedCVPDF, FixedCVGenerator };
