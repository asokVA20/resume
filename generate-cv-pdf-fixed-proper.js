...
// Fixed Professional CV Design Configuration
const CV_CONFIG = {
    page: {
        margin: 50,
...
// Main function to generate fixed proper PDF
function generateFixedProperCVPDF() {
    try {
        // Load personal data
        const data = JSON.parse(fs.readFileSync('personal-data.json', 'utf8'));
        
        // Create CV generator
        const cvGenerator = new FixedProperCVGenerator(data);
        
        // Generate PDF
        const doc = cvGenerator.generate();
        
        // Save to file
        const outputPath = 'cv-anthony-okala-fixed-proper.pdf';
        doc.pipe(fs.createWriteStream(outputPath));
        doc.end();
        
        console.log('✅ Fixed Proper CV PDF generated successfully!');
        ...
        console.error('❌ Error generating fixed proper CV PDF:', error.message);
        throw error;
    }
}

// Export for use in other files
module.exports = { generateFixedProperCVPDF, FixedProperCVGenerator };

// Run if called directly
if (require.main === module) {
    generateFixedProperCVPDF();
}
