...
// Professional CV Design Configuration based on LaTeX template
const CV_CONFIG = {
    page: {
        margin: 50,
...
// Main function to generate proper PDF
function generateProperCVPDF() {
    try {
        // Load personal data
        const data = JSON.parse(fs.readFileSync('personal-data.json', 'utf8'));
        
        // Create CV generator
        const cvGenerator = new ProperCVGenerator(data);
        
        // Generate PDF
        const doc = cvGenerator.generate();
        
        // Save to file
        const outputPath = 'cv-anthony-okala-proper.pdf';
        doc.pipe(fs.createWriteStream(outputPath));
        doc.end();
        
        console.log('✅ Proper CV PDF generated successfully!');
        ...
        console.error('❌ Error generating proper CV PDF:', error.message);
        throw error;
    }
}

// Export for use in other files
module.exports = { generateProperCVPDF, ProperCVGenerator };
